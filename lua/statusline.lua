...
vim.api.nvim_create_autocmd("LspProgress", {
	...
	callback = function(args)
		-- Check if args.data exists to avoid nil error
		if not args.data then
			return
		end

		-- Safely get the client name by checking if the client exists
		local client = vim.lsp.get_client_by_id(args.data.client_id)
		if not client then
			return
		end

		-- Safely access result and its nested properties
...
function M.diagnostics_component()
	...
	for severity, count in pairs(counts) do
		if count > 0 then
			...
			table.insert(parts, string.format("%%##%s %d", icons.diagnostics[severity], count))
		end
	end
...
function M.filetype_component()
	...
	return string.format("%%##%s %%#StatuslineTitle#%s", icon, filetype)
end
